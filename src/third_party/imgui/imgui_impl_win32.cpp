...
#include "imgui.h"
	...
	// #define IMGUI_IMPL_WIN32_DISABLE_GAMEPAD              // Disable gamepad support. This was meaningful before
	// <1.81 but we now load XInput dynamically so the option is now less relevant.
...
void ImGui_ImplWin32_NewFrame() {
	...
	// There is no distinct VK_xxx for keypad enter, instead it is VK_RETURN + KF_EXTENDED, we assign it an arbitrary
	// value to make code more readable (VK_ codes go up to 255)
...
static ImGuiKey ImGui_ImplWin32_VirtualKeyToImGuiKey(WPARAM wParam) {
	...
	// Call from your application's message handler. Keep calling your message handler unless this function returns
	// TRUE. When implementing your own backend, you can read the io.WantCaptureMouse, io.WantCaptureKeyboard flags to
	...
	// them from your application based on those two flags. PS: In this Win32 handler, we use the capture API
	// (GetCapture/SetCapture/ReleaseCapture) to be able to read mouse coordinates when dragging mouse outside of our
	// window bounds. PS: We treat DBLCLK messages as regular mouse down messages, so this code will work on windows
	// classes that have the CS_DBLCLKS flag set. Our own example app code doesn't set this flag.
	#if 0
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_XBUTTONDBLCLK: {
		...
		if (bd->MouseButtonsDown == 0 && ::GetCapture() == nullptr) ::SetCapture(hwnd);
...
void ImGui_ImplWin32_EnableDpiAwareness() {
	...
	::SetProcessDPIAware();
		...
		#pragma comment(                                                                                               \
			lib,                                                                                                       \
			"gdi32"                                                                                                    \
		) // Link with gdi32.lib for GetDeviceCaps(). MinGW will require linking with '-lgdi32'
	#endif
