...
struct has_global_insertion_operator<
	Value,
	decltype(::operator<<(declval<std::ostream&>(), declval<const Value&>()), void())> : types::true_type {};

template <typename Value, typename = void>
struct has_insertion_operator {
	static DOCTEST_CONSTEXPR bool value = has_global_insertion_operator<Value>::value;
};
...
struct StringMakerBase {
	...
	static String convert(const DOCTEST_REF_WRAP(T)) {
		...
		static_assert(
			deferred_false<Value>::value,
			"No stringification detected for type Value. See string conversion manual"
		);
#endif
		return "{?}";
	}
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
	template <typename Value>
	typename std::enable_if<std::is_constructible<double, Value>::value, Approx&>::type epsilon(const Value& newEpsilon
	) {
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
	template <typename Value>
	typename std::enable_if<std::is_constructible<double, Value>::value, Approx&>::type scale(const Value& newScale) {
	...
	#define DOCTEST_REQUIRE_UNARY_FALSE(...) DOCTEST_UNARY_ASSERT(DT_REQUIRE_UNARY_FALSE, __VA_ARGS__)
		...
		#define DOCTEST_ASSERT_THROWS_WITH(expr, expr_str, assert_type, ...)                                           \
			DOCTEST_FUNC_SCOPE_BEGIN {                                                                                 \
				if (!doctest::getContextOptions()->no_throw) {                                                         \
					doctest::detail::ResultBuilder                                                                     \
						DOCTEST_RB(doctest::assertType::assert_type, __FILE__, __LINE__, expr_str, "", __VA_ARGS__);   \
					try {                                                                                              \
						DOCTEST_CAST_TO_VOID(expr)                                                                     \
					} catch (...) {                                                                                    \
						DOCTEST_RB.translateException();                                                               \
		...
		#define DOCTEST_EXCEPTION_EMPTY_FUNC                                                                           \
			[] {                                                                                                       \
				static_assert(                                                                                         \
					...
					"Use DOCTEST_CONFIG_NO_EXCEPTIONS_BUT_WITH_ALL_ASSERTS if you want to compile with exceptions "    \
					"disabled."                                                                                        \
				);                                                                                                     \
				return false;                                                                                          \
			}()
...
struct ConsoleReporter : public IReporter {
	...
	void printHelp() {
		...
		// clang-format off
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "op,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file=<filters>         "
...
void Context::parseArgs(int argc, const char* const* argv, bool withDefaults) {
    ...
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "op=",                 p->filters[0]);
