...
public:
	...
	// Range accessors
	SF_ALWAYS_INLINE T* begin() const { return m_ptr; }
	SF_ALWAYS_INLINE T* end() const { return m_ptr + m_len; }
	SF_ALWAYS_INLINE T* cbegin() const { return m_ptr; }
	SF_ALWAYS_INLINE T* cend() const { return m_ptr + m_len; }

	// Accessor
	SF_ALWAYS_INLINE T& operator[](usize index) const {
	...
	SF_ALWAYS_INLINE usize shrink(usize amount) {
		SF_ASSERT(amount <= m_len, "Can not shrink more than len");
		m_len -= amount;
		return len();
	}
...
public:
	...
	// Range accessors
	SF_ALWAYS_INLINE T const* begin() const { return m_ptr; }
	SF_ALWAYS_INLINE T const* end() const { return m_ptr + m_len; }
	SF_ALWAYS_INLINE T const* cbegin() const { return m_ptr; }
	SF_ALWAYS_INLINE T const* cend() const { return m_ptr + m_len; }

	// Accessor
	SF_ALWAYS_INLINE T const& operator[](usize index) const {
		SF_ASSERT(is_valid_index(index), "Index out of bounds.");
		return m_ptr[index];
	}
