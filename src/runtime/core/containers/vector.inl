...
template <typename Element>
SF_ALWAYS_INLINE bool Vector<Element>::is_valid_index(usize index) const {
	return index < len();
}

template <typename Element>
SF_ALWAYS_INLINE Vector<Element>::operator Slice<Element>() {
	return { m_ptr, m_len };
}

template <typename Element>
SF_ALWAYS_INLINE Vector<Element>::operator Slice<Element const>() const {
	return { m_ptr, m_len };
}

template <typename Element>
SF_ALWAYS_INLINE Element* Vector<Element>::begin() {
	return m_ptr;
}

template <typename Element>
SF_ALWAYS_INLINE Element* Vector<Element>::end() {
	return m_ptr + m_len;
}

template <typename Element>
SF_ALWAYS_INLINE const Element* Vector<Element>::cbegin() const {
	return m_ptr;
}

template <typename Element>
SF_ALWAYS_INLINE const Element* Vector<Element>::cend() const {
	return m_ptr + m_len;
}

template <typename Element>
SF_ALWAYS_INLINE Element& Vector<Element>::operator[](usize index) {
	SF_ASSERT(is_valid_index(index), "Index out of bounds");
	return m_ptr[index];
}

template <typename Element>
SF_ALWAYS_INLINE const Element& Vector<Element>::operator[](usize index) const {
	SF_ASSERT(is_valid_index(index), "Index out of bounds");
	return m_ptr[index];
}

template <typename Element>
SF_ALWAYS_INLINE Option<Element&> Vector<Element>::last() {
	if (len() > 0) return m_ptr[len() - 1];
	return nullptr;
}

template <typename Element>
SF_ALWAYS_INLINE Option<Element const&> Vector<Element>::last() const {
	if (len() > 0) return m_ptr[len() - 1];
	return nullptr;
}

template <typename Element>
SF_ALWAYS_INLINE void Vector<Element>::reserve(usize amount) {
...
template <typename Element>
SF_ALWAYS_INLINE void Vector<Element>::insert(usize index, const Element& item) {
	Element copy = item;
	insert(index, sf::move(copy));
}

template <typename Element>
SF_ALWAYS_INLINE usize Vector<Element>::push(Element&& item) {
...
template <typename Element>
SF_ALWAYS_INLINE usize Vector<Element>::push(const Element& item) {
	Element copy = item;
...
Element Vector<Element>::remove(usize index) {
	...
	auto result = sf::move(m_ptr[index]);
...
template <typename Element>
SF_ALWAYS_INLINE Option<Element> Vector<Element>::pop() {
	if (m_len > 0) {
		m_len -= 1;
		return sf::move(m_ptr[m_len]);
	}
	return nullptr;
}
