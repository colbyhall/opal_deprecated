...
template <typename T>
GJ_ALWAYS_INLINE Atomic<T>::Atomic(Atomic&& rhs) noexcept : m_atomic(rhs.m_atomic.load(std::memory_order_relaxed)) {}
...
template <typename T>
GJ_NO_DISCARD GJ_ALWAYS_INLINE T Atomic<T>::exchange(T desired, Order order) const noexcept {
...
template <typename T>
GJ_NO_DISCARD GJ_ALWAYS_INLINE T Atomic<T>::fetch_add(T arg, Order order) const noexcept {
	return m_atomic.fetch_add(arg, to_std(order));
}

template <typename T>
GJ_NO_DISCARD GJ_ALWAYS_INLINE T Atomic<T>::fetch_sub(T arg, Order order) const noexcept {
	return m_atomic.fetch_sub(arg, to_std(order));
}

template <typename T>
GJ_NO_DISCARD GJ_ALWAYS_INLINE T Atomic<T>::fetch_and(T arg, Order order) const noexcept {
	return m_atomic.fetch_and(arg, to_std(order));
}

template <typename T>
GJ_NO_DISCARD GJ_ALWAYS_INLINE T Atomic<T>::fetch_or(T arg, Order order) const noexcept {
	return m_atomic.fetch_or(arg, to_std(order));
}

template <typename T>
GJ_NO_DISCARD GJ_ALWAYS_INLINE T Atomic<T>::fetch_xor(T arg, Order order) const noexcept {
